  ...
  >
    <div class="theme-board" data-testid="theme-board-view">
      ...
      <section v-else-if="phase === 'empty'" class="theme-board__state theme-board__state--create">
        ...
        <p>Open this route with an explicit builderProjectId, or create one persistent draft.</p>
...
async function createProject(): Promise<void> {
  ...
  await router.replace({ name: "dev-theme-board", query: { builderProjectId: result.data.builderProjectId } });
}
...
.theme-board__form { display: grid; width: min(100%, 620px); gap: 12px; }
